...
setup_plugin() {
	if [ "${SKIP_WP_SETUP}" = "true" ]; then
		echo "Skipping wp-graphql-rank-math installation..."
		return 0
	fi

	# Add this repo as a plugin to the repo
	if [ ! -d $WP_CORE_DIR/wp-content/plugins/wp-graphql-rank-math ]; then
		ln -s $PLUGIN_DIR $WP_CORE_DIR/wp-content/plugins/wp-graphql-rank-math
